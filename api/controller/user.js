...
const user = {
    adduser: async(req,res)=>{
       ...
       try{
      const person = await userService.InsertIntoTable(req.body.username,req.body.lastname,req.body.IsActive, req.body.firstname, req.body.email, req.body.password);
    ...
    login: async(req,res)=>{
      try{
      const accesToken = jwt.sign({id:req.body.insertId},process.env.ACCESS_TOKEN_SECRET, { expiresIn: '10m' });
      const refreshToken =  jwt.sign({id:req.body.insertId},process.env.REFRESH_TOKEN_SECRET)
